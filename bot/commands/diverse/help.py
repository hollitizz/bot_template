...
def getCommandFormattedEmbed(ctx: Interaction, command: app_commands.Command, group_name: str):
    embed: Embed = Embed(title=group_name, url="",
                         description=f"Help for the command `{command.name}`", color=0xff4013)
    embed.set_author(name="Hollitizz", url="https://discordapp.com/users/222008900025581568",
                     icon_url="https://cdn.discordapp.com/avatars/222008900025581568/e4bc396801ccadf364d2907869404844.png")
    ...
    embed.add_field(
        name="Usage", value=f"/{command.name} {params}", inline=False)
    return embed


class drawHelpSelect(ui.Select):
    def __init__(self, items, cog_name):
        self.cogs_options = [SelectOption(
            label=item.name, value=i) for i, item in enumerate(items)]
        super().__init__(
...
class drawCogSelect(ui.Select):
    def __init__(self, items):
        self.cogs_options = [SelectOption(
            label=f"{item_name}", value=item_name) for item_name, item in items.items()]
        super().__init__(
